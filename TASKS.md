...
### Cities & sourcing ✅ 2026-08-05
- [x] **A city is LIVE when a source feeds it** — `lib/cities.ts` derives it from `lib/sources/registry.ts`; the hand-typed `status: "live"` in `prototype-data` is no longer read by anything real. `SourceDef` gained `city` + `enabled`. Proven by temporarily adding one Oakland source: live list became SF + Oakland, and a viewer in Oakland flipped from "we're not in Oakland yet" to "Oakland". **Adding Oakland = add Oakland sources, nothing else.**
- [x] **Admin → Sourcing shows the real pipeline** (`components/admin/SourcingPanel.tsx` + `app/api/admin/sources`). It was rendering the prototype's six invented sources with invented run stats, so the screen had shipped while the wiring never had.
- [x] **`somarts` shows 0 events and is FINE** — checked, don't re-flag it. The recipe works (`npx tsx scripts/publish-events.ts --only somarts --dry-run` → pulled 1, kept 1, placed 1, would publish). SOMArts simply lists one event right now (16 Oct) and listed nothing when the table was seeded on 2026-08-04. **A zero in the Sourcing panel means "nothing to write", not "broken"** — check the source's own page before assuming drift.
- [ ] **Adding a source is still a code edit + deploy.** A source is a scraping recipe (selectors, venue templates, drift baselines), not a form row. Making it DB-driven (`event_sources`) is the next step whenever that friction actually bites.
- [ ] The prototype at `/prototype/admin` still exists with its mock data. Nothing links to it from the real admin any more; delete it when nothing else references `prototype-data`.
