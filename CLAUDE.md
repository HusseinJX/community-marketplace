...
Also: `TECH-DEBT.md` (deferred problems + why), `features/` (forward roadmap specs — newest: `marketplace-events.md` the webhook contract, `memberships-wallet.md` the membership model), `docs/vendor-setup-guide.md` + `docs/phone-forwarding-by-carrier.md` (client-facing), `session-context/` (session write-ups), `CHANGELOG.md`.
...
- **Tags** `/tags/[slug]` — where a business is and what it was part of. Public to read, ADMIN to write: applied on the canvass review screen while onboarding, or edited on any business profile. `lib/tags.ts`.
- **Public art** `/vendor/admin` → Public art (super-admin) — photograph a mural, tag the location, credit every artist; create an artist inline with a web-researched profile. Artists are unclaimed `memberType: "artist"` members and land in the Artists group of browse automatically.
- **Live canvass** `/joindemo/live` — password-gated, ADMIN-only writes: search a business on Google Maps, read a web-researched story on screen, create their **unclaimed** profile. `/joindemo` (the side-effect-free walkthrough) is a separate door and writes nothing.
...
- **Tags: bounded THING → dates on the tag; bounded EDGE → dates on `member_tags`.** "Outside Lands 2026" is one week, so the dates live on the tag and everyone tagged inherits them. Ferry Plaza is permanent but a vendor is only there Saturdays, so THAT lives on the membership (`recurrence`/`role`). A business inside Oracle Park has neither. "How permanent is this tag" felt unanswerable because it is two independent axes, not one — never encode it in the tag's name. Several locations of one business stay several member profiles, linked by a tag of `kind: brand` whose page IS the list of locations.
- **A partial unique index cannot back `ON CONFLICT`** through PostgREST — it doesn't repeat the predicate, and every upsert fails with "no unique or exclusion constraint matching". `member_tags_member_tag_idx` was `WHERE tag_id IS NOT NULL` and silently broke every tag write. Plain is usually fine anyway: Postgres treats NULLs as distinct.
- **Count what LANDED, not what was asked for.** The canvass screen said "Tagged into 1 tag" while every tag write was failing, because the count came from `tags.length`. Same shape as the memberships Join button that 404'd: a success message sourced from intent instead of result.
- **A post can credit more than one member — read BOTH places.** `posts.tagged_member_id` holds the single/primary tag; `post_member_tags` holds the full list for multi-tag posts (a mural's artists). `getPostsByMemberId` unions them. Anything new that asks "is this member in this post?" must check both, or an artist silently loses every wall they shared.
...
- **2026-09-23 — v145 + two specs, no code.** The onboarding doors (`/joindemo/live`, `/joindemo`) are linked from super-admin at last. Then two designs settled on paper because the expensive part is the decision: **`features/marketplace-events.md`** (the webhook contract — event names are permanent the moment a vendor points an automation at one, and **deliveries key on the object's TRANSITION not the code path**, because `order.paid` is written in two places on purpose and per-site emits would fire twice on some orders and once on others) and **`features/memberships-wallet.md`** (**memberships are a MARKETPLACE, not an aggregator** — you buy here so it is here, which makes "all your local memberships in one place" true by construction; a first draft with linked memberships and a three-level trust ladder was deleted because a `self_declared` membership could redeem nothing). Also: `npm test` had been **red for a month** — `tests/ai.test.ts` scored the assistant's correct refusal to invent a price as a failure, after its seeded product was deactivated on 2026-08-22. 37/37 now.
- **2026-09-23 — v144: tags, and the rule that makes them answerable.** A tag is a ROW now (`tags`) rather than a slug on a join, so it carries a kind, a place, dates and a parent — and **time lives on the tag when the THING is bounded, on the edge when the RELATIONSHIP is bounded** (see Conventions). Public to read, admin to write; applied on the canvass review screen (and surviving "Next spot", because the next vendor is in the same market) or edited on any business profile, which is also the first UI for `role`/`recurrence`. Two bugs found by using it: a **partial unique index can't back `ON CONFLICT`**, so every tag write was failing, and the screen **said it had worked** because it counted intent instead of results.
- **2026-09-23 — v143: public art, and a post that can credit more than one person.** Super-admin → Public art photographs a mural, tags the location and credits every artist, creating the ones who aren't here yet with a web-researched profile (read it BEFORE it writes, same shape as the canvass tool). The real work is **`post_member_tags`**: `posts.tagged_member_id` is ONE column, and overwriting it with the "main" artist quietly decides the others didn't paint it. **The old column stays authoritative for single tags** — every existing post and the whole share composer are untouched — and member-scoped reads union the two, with the first artist also written to the legacy column so single-tag surfaces still show a credit. Tags are batched into one query beside reactions. Verified live: one artwork, two real SF muralists, on both their profiles.
- **2026-09-23 — v142: canvassing became a thing you can do live.** `/joindemo/live` turns "what's your favourite local spot?" into a real unclaimed profile while the person is still talking — Places listing, a web-researched story shown **before** anything is written, duplicate check, then create. **Reading and writing are gated differently on purpose**: lookup runs on the /joindemo password like the other read-only billable routes, but create needs a Clerk ADMIN, because it puts a row in the public directory a real business may later claim. The client sends only a `placeId` — every fact is re-fetched and the story re-researched server-side, since a client-supplied "story" would be unattributed text on a real business's page. The phone lands in `businessPhone`, **never `trustedPhone`**: that field is the OTP anchor for claiming, and these profiles exist to be claimed. `researchListing` gained a `timeoutMs` — 11s is right for `/join` (which can proceed without research) and wrong here (where the story IS the demo); a cold connector took 7s+ and the default lost it on the first real run.
- **2026-09-23 — v141: home was asking for the same feed twice.** `usePersonalizedEvents` keyed on a body containing `tasteId`, which is only knowable in the browser and so arrives a render late — first render keyed on `null`, second on the real id, and SWR correctly fired both. **383KB and ~2.6s on every home visit, half of it thrown away.** The hook takes a `ready` gate; the caller tracks whether the localStorage read HAPPENED, not whether it returned something (both states are null, only one means "don't ask yet"). Verified on prod: one call. Separately `/api/events/feed` dropped 119KB→104KB by shipping 200-char descriptions — but **the theme is classified server-side from the full text first**, because `themeOf()` keyword-matches across title + description and truncating alone silently re-filed **22 of 120 events**. And the cookie banner, which sat 3.5rem up to clear a `md:hidden` bottom nav, stopped parking itself on top of desktop cards.
...
- **2026-08-22 — four buttons, and a first post.** The vendor dashboard's top level is **Shop · Posts · Events · Profile** (`components/vendor/VendorHome.tsx`); those four dropped out of the tile list below, everything else stayed. Onboarding's last screen leads with "Make your first post" (`/share?vendor=1`), dashboard as the link beneath. And the rule that came out of it: **a surface everyone can reach must state its price** — Events is reachable by every vendor but `organizeEvents` is Member ($10), and `EventsManager` was discarding the 402 so Save silently did nothing. It renders `UpgradePrompt` now. Free = profile, posts, discovery, receiving invites, commerce. **Selling is free; hosting is not.**
